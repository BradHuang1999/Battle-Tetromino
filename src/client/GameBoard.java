...
public class GameBoard extends JPanel{
    protected boolean[][] map = new boolean[14][25];
    protected boolean[][] disappearable = new boolean[14][25];
    protected boolean[][] movable = new boolean[14][25];
    protected Color[][] colors = new Color[14][25];

    protected Tetromino current;

    protected int piecePositionX, piecePositionY;
    ...
    public void replace(Tetromino tetromino){
    ...
    private int replacePossible(boolean[][] tetrominoShape){
        ...
        for (int i = 0; i < 4; i++){
            ...
            for (int x = 0; x < 4; x++){
                for (int y = 0; y < 4; y++){
                    try {
                        if (tetrominoShape[y][x] && (!this.movable[pieceX + x][this.piecePositionY + y] && this.map[pieceX + x][this.piecePositionY + y])){
                            returnable = false;
                            break outerLoop;
                        }
    ...
    public void newTetromino(Tetromino tetromino){
        ...
        for (int y = 0; y < 4; y++){
            for (int x = 0; x < 4; x++){
                if (tetromino.getCurrentShape()[y][x]){
